...
if(isset($_POST['signup'])){
    ...
    if(count($errors) === 0){
        ...
        if($data_check){
            ...
            $sender = "From: ravigeoyadav@gmail.com";
            if(mail($to, $subject, $message, $sender)){
                $info = "We've sent a verification code to your email - $email";
                $_SESSION['info'] = $info;
                $_SESSION['email'] = $email;
                $_SESSION['password'] = $password;
                header('location: Adminuser-otp.php');
                exit();
            }else{
    ...
    if(isset($_POST['check'])){
        ...
        if(mysqli_num_rows($code_res) > 0){
            ...
            if($update_res){
                $_SESSION['name'] = $name;
                $_SESSION['email'] = $email;
                header('location: Adminsignup-user.php');
                exit();
            }else{
    ...
    if(isset($_POST['check-email'])){
        ...
        if(mysqli_num_rows($run_sql) > 0){
            ...
            if($run_query){
                $email = "virubhai5348@gmail.com";
                $subject = "Password Reset Code";
                $message = "Your password reset code is $code";
                $header = "From: ravigeoyadav@gmail.com";
                if(mail($email, $subject, $message, $header)){
                    $info = "We've sent a passwrod reset otp to your email - $email";
                    $_SESSION['info'] = $info;
                    $_SESSION['email'] = $email;
                    header('location: Adminreset-code.php');
                    exit();
                }else{
                    $errors['otp-error'] = "Failed while sending code!";
                }
    ...
    if(isset($_POST['check-reset-otp'])){
        ...
        if(mysqli_num_rows($code_res) > 0){
            ...
            $_SESSION['info'] = $info;
            header('location: Adminnew-password.php');
            exit();
        }else{
            $errors['otp-error'] = "You've entered incorrect code!";
        }
    ...
    if(isset($_POST['change-password'])){
        ...
        if($password !== $cpassword){
            ...
            if($run_query){
                ...
                $_SESSION['info'] = $info;
                header('Location: Adminpassword-changed.php');
            }else{
